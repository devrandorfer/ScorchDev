...
Function Invoke-SPORestMethod {
    ...
    $SPOCredential = New-Object -TypeName Microsoft.SharePoint.Client.SharePointOnlineCredentials($Credential.UserName, $Credential.Password) 
    ...
    Foreach($HeaderKey in ($Headers.Keys -as [array]))
    {
    ...
    if(($results.d | Get-Member -MemberType NoteProperty).Name -contains 'Results')
    {
        $ReturnValue = $results.d.results
    }
    else
    {
        $ReturnValue = $results.d
    }

    Write-CompletedMessage @CompletedParams -Status ($ReturnValue | ConvertTo-JSON)
...
<#
    ...
    .Example Site
        Format-SPOUri -SPOFarm 'solutions.contoso.com' `
        -SPOSite 'gcloud'
    ...
    .Example List
        Format-SPOUri -SPOFarm 'solutions.contoso.com' `
        -SPOSite 'gcloud' `
        -SPOList 'AddDiskToAVirtualServer'
    ...
    .Example List with Filter
        ...
        -SPOSite   'gcloud' `
        -SPOList   'AddDiskToAVirtualServer' `
        -SPOFilter 'StatusValue eq Failed'
...
Function Get-SPOList
{
    ...
    $null = $(
        if(-not $SPUri)
        {
            $SPUri = Format-SPOUri -SPFarm $SPFarm -SPSite $SPSite -SPCollection $SPCollection -UseSSl $UseSSl
        }

        $returnLists = New-Object -TypeName System.Collections.ArrayList

        $ListService = Invoke-SPORestMethod -Uri $SPUri -Credential $Credential
...
<#
    ...
    .EXAMPLE
        ...
        Get-SPOListItem -SPOURI $SPListURI -Credential $SPCred

        .EXAMPLE
        Get all list items from a SharePoint list

        Get-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -Credential $SPCred
        ...
        Get-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -Credential $SPCred -Filter $SPFilter

    .EXAMPLE
        Expand a linked property such as a linked user

        Get-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -ExpandProperty CreatedBy

    .EXAMPLE
        Expand all linked properties

        Get-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -ExpandProperty *
#>
Function Get-SPOListItem
{
    ...
    Param( 
        ...
        [Parameter(Mandatory = $True)][PSCredential] $Credential = $Null,
    ...
    $null = $(
        if( -not $SPUri )
        {
            $SPUri = Format-SPOUri -SPFarm       $SPFarm `
                                  -SPSite       $SPSite `
                                  -SPList       $SPList `
        ...
        $MoreItems = Invoke-SPORestMethod -Uri $SPUri -Credential $Credential
        $RawList = @()
        ...
        while ( $MoreItems )
        {
            ...
            { 
                ...
                { 
                    ...
                    $MoreItems = Invoke-SPORestMethod -Uri $PageURI -Credential $Credential
                }
                else
                {
                    break
        ...
        {
            $SPListItem = ConvertFrom-RawSPOItem -ListItem $ListItem -Immutable $False
            ...
            foreach($Property in $ExpandLinks)
            {
                if($Property -eq 'Attachments')
                {
                    ...
                    {
                        $LinkedItem = Get-SPOListItemAttachment -SPUri $SPListItem.Id `
                ...
                {
                    $LinkedItem = Get-SPOListItemImmutable -SPUri "$($SPListItem.Id)/$Property" -Credential $Credential
                }
...
<#
    .SYNOPSIS
        ...
        This function is an internal function used by Get-SPOListItem to return immutable
    ...
    .PARAMETER Credential
        Credential with rights to query SharePoint list. If not used default credentials will be used
#>
Function Get-SPOListItemImmutable
{
...
#>
Function New-SPOListItemObject
{
    Param(
    ...
    $ASMXWebservicePath = Format-SPOListASMXPath -ListItemID $Id
    $SPListItemProperties = Split-SPOListItemId -SPListItemID $Id
...
<#
    ...
    .PARAMETER AttachmentFormat
        ...
        ASCII: Ascii formatting
#>
Function Get-SPOListItemAttachment
{
    Param(
...
#>
Function ConvertFrom-RawSPOItem
{
    Param ( 
        [Parameter(Mandatory = $True) ]
        $ListItem,
    ...
    $null = $(
        $Id = $ListItem.__metadata.uri
        $ListItemPropertyNames = ( $ListItem | Get-Member -MemberType NoteProperty ).Name

        if($ListItemPropertyNames.Contains('Created')) {  }
        else { $ListItemCreated = [System.DateTime]::MinValue }

        if($ListItemPropertyNames.Contains('Modified')) { $ListItemModified = $ListItem.Modified -as [datetime] }
        else { $ListItemModified = [System.DateTime]::MinValue }

        if($ListItemPropertyNames.Contains('Version')) { $ListItemVersion = $ListItem.Version }
        ...
        $SPListItem = New-SPOListItemObject -Id $Id -Created $ListItemCreated -Modified $ListItemModified `
...
<#
    ...
    .EXAMPLE
        ...
        Update-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -SPOListItemIndex 7 -Data $Data -Credential $SPCred
    ...
    .EXAMPLE
        ...
        $Data = @{ 'LastRun' = (Get-Date); 'Status' = $FinalStatus }
        Update-SPOListItem -SPOUri $SPUri -Data $Data -Credential $SPCred

    .EXAMPLE
        Update SharePoint list items assigned to the given team with the new team name

        $SPFilter = "Team eq 'WE-Apps'"
        $ListItems = Get-SPOListItem -SPOFarm $SPFarm -SPOSIte $SPSite -SPOList $SPList -Filter $SPFilter -Credential $SPCred
        Foreach($ListItem in $ListItems)
        {
            $ListItem.Properties.Team = 'Web Hosting'
            Update-SPOListItem -SPOListItem $ListItem -Credential $SPCred
        }
#>
Function Update-SPOListItem
{
    ...
    $null = $(
        If($SPListItem)
        {
            ...
            $CurrentSPListItem = Get-SPOListItem -SPOUri $SPUri -Credential $Credential
            $Data = @{}
        ...
        ElseIf($SPFarm)
        {
            $SPUri = "$(Format-SPOUri -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -SPOCollection $SPCollection -UseSSl $UseSsl)($SPListItemIndex)"
        }
        ElseIf($SPUri)
        {
        ...
        Else
        {
            Write-Error -Message "You must pass one of the following parameter sets`n
                                 -SPOUri `$SPUri -Data `$Data
                                 -SPOFarm `$SPFarm -SPOSite `$SPSIte -SPOList `$SPList -SPOListItemIndex
                                 `$SPListItemIndex -Data `$Data`n
                                 -SPOListItem `$SPListITem"
        }
        ...
        if($PassThru) { $returnItem = Get-SPOListItem -SPOUri $SPUri -Credential $Credential }
    )
...
<#
    ...
    $Data = @{ 'LastRun' = (Get-Date); 'Status' = $FinalStatus }
        Add-SPOListItem -SPOFarm $SPFarm -SPOSite $SPSite -SPOList $SPList -Data $Data
#>
Function Add-SPOListItem
{
    ...
    if(-not $SPUri)
    {
        $SPUri = Format-SPOUri -SPOFarm $SPFarm -SPOSite $SPSite -SPOCollection $SPCollection -SPOList $SPList -UseSSl $UseSsl
    }
    $UpdateData = @{}
...
Function Remove-SPOListItem
{
    ...
    $null = $(
        ...
        ElseIf($SPFarm)
        {
            ...
            $SPUri = "$(Format-SPOUri -SPOFarm $SPFarm -SPOSite $SPSite -SPOCollection $SPCollection -SPOList $SPList -UseSSl $UseSsl)($SPListItemIndex)"
        }
        ElseIf($SPUri)
        {
            # SPUri was passed no processing to do
...
<#
    ...
    .PARAMETER SharePointPerson
        A Sharepoint list item describing a person (i.e. a LinkedItem describing a person from Get-SPOListItem)

    .EXAMPLE
        $ListItem = Get-SPOListItem -SPOUri $Uri -ExpandProperty 'CreatedBy'
...
<#
    ...
    .EXAMPLE
        Test-SPOListItemStartTime -SPOUri $NewRequestUri -MaximumSecondOffset 900
#>
Function Test-SPOListItemStartTime
{
    [OutputType([bool])]
    Param(
        [Parameter(Mandatory = $True)]
    ...
    $null = $(
        ...
        $ListItem = Get-SPOListItem @GetSPListItemParameters
...
<#
    ...
    .Example
        Add-SPOListItemAttachment -SPOListItem $SPListItem -Credential $SPCred -AttachmentPath 'C:\temp\blah.xlsx'
#>
Function Add-SPOListItemAttachment
{
    Param(
        [Parameter(ParameterSetName = 'ExplicitURI', Mandatory = $True)]
        [string]
    ...
    $null = $(
        ...
        {
            # If a whole item is passed in compare delete it based on its Id
            $SPListItem = Get-SPOListItem -SPOUri $SPUri
        }
        ElseIf($SPFarm)
        {
            # If the BuildURI Parameter set is passed created the correct SPUri
        ...
        $ASMXListItem = Get-ASMXListItem -SPOListItem $SPListItem `
                                         -Credential $Credential
        $WebserviceProxy = New-SharepointASMXWebserviceProxy -SPOListItem $SPListItem `
                                                             -Credential $Credential
...
#>
Function Format-SPOListASMXPath
{
    [OutputType([string])]
    Param(
        [Parameter(Mandatory = $True)]
...
Export-ModuleMember -Function *-SPO* -Verbose:$false